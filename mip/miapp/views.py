...
def acerca(request):
    return HttpResponse(
        """
        <html>
            <h1>Acerca de mi ... mi historia ... </h1>
            <h2>Profesor Norman Beltran </h2>
            <h3>Curso de Desarrollo Web con Python & Django</h3>
        </html>
        """
    )
...
def bienvenido2(request):
    ...
    ctx = {"nombre": "Django / Python", "alumnos": 40, 
           "comision": 88888, "dias": "Martes, Miercoles y Jueves",
           "notas": [7,8,9,4,5,6,10]}
...
def unCurso(request, id):
    ...
    try:
        ...
        conn.close()
        raise Http404
        #return HttpResponse(f"El curso {id} no existe en la BD")
